...
#FWEXTIP2=192.168.1.31
IPCLIENT1=10.10.10.134
#IPCLIENT2=10.10.10.20
...
iptables -t nat -A PREROUTING -i $IE -s $EXT -d $FWEXTIP1 -j DNAT --to $IPCLIENT1
iptables -A FORWARD -i $IE -j ACCEPT
iptables -A FORWARD -s $FWEXTIP1 -j ACCEPT
iptables -A FORWARD -s $IPCLIENT1 -j ACCEPT

# Enabling network address translation
iptables -t nat -A POSTROUTING -o $IE -s $NETLAN -j SNAT --to-source $FWEXTIP1
...
#iptables -A ei -p tcp --dport 22 -m state --state NEW,ESTABLISHED -j ACCEPT
#iptables -A ie -p tcp --sport 22 -m state --state ESTABLISHED -j ACCEPT
...
# Forward POP3
iptables -A ie -p tcp --dport 103 -m state --state NEW,ESTABLISHED -j ACCEPT
iptables -A ei -p tcp --sport 103 -m state --state ESTABLISHED -j ACCEPT
